...
const compileExclusionRules = memoize(parseExclusionRules);

export function contentExcluded(content: string | null | undefined, guildId?: string | null, channelId?: string | null, authorId?: string | null): boolean {
    if (content == null) return false;
    return messageMatchesRules(compileExclusionRules(settings.store.exclusionRules), { content, guildId, channelId, authorId });
}
...
*/
export function shouldIgnore({ channelId, authorId, guildId, flags, bot, ghostPinged, content }: ShouldIgnoreArguments): boolean {
    ...
    if (guildId != null && shouldIgnoreMutedGuilds && UserGuildSettingsStore.isMuted(guildId)) return true;
    if (channelId != null && shouldIgnoreMutedCategories && UserGuildSettingsStore.isCategoryMuted(guildId, channelId)) return true;
    if (channelId != null && shouldIgnoreMutedChannels && UserGuildSettingsStore.isChannelMuted(guildId, channelId)) return true;
...
export type ListMenuState = "remove" | "move" | "add";

export function getListMenuState(listType: ListType, ids: string[]): ListMenuState {
    const inList = getIdList(listType);
    const oppositeListType = listType === "blacklistedIds" ? "whitelistedIds" : "blacklistedIds";
    const inOpposite = getIdList(oppositeListType);

    if (ids.every(id => inList.includes(id))) return "remove";
    if (ids.some(id => inOpposite.includes(id))) return "move";
    return "add";
}
export function getIdList(list: ListType): string[] {
    return ((settings.store[list] as string) ?? "").split(",").map(e => e.trim()).filter(Boolean);
}
