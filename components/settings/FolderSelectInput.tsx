...
export const ImageCacheDir = createDirSelector("imageCacheDir", () => t("folder.imageCacheUpdated"));
export const LogsDir = createDirSelector("logsDir", () => t("folder.logsDirUpdated"));
    ...
    return (
        <div className={classes(cl("-container"), inputClasses.input)}>
            <div onClick={() => copyWithToast(path)} className={cl("-input")}>
                {path == null || path === DEFAULT_IMAGE_CACHE_DIR ? t("folder.chooseFolder") : getDirName(path)}
            </div>
            <Button
                className={cl("-button")}
                size="small"
